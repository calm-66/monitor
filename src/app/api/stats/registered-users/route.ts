...
function buildRegisteredUsersUrl(domain: string | null, date: string): string | null {
  if (!domain) return null;
  const cleanedDomain = domain.trim().replace(/^https?:\/\//i, '').replace(/\/+$/, '');
  if (!cleanedDomain) return null;
  return `https://${cleanedDomain}/api/monitor/registered-users?date=${encodeURIComponent(date)}`;
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = buildRegisteredUsersUrl(project.domain, date);
    if (!url) {
      return NextResponse.json(
    ...
    log('error', 'Failed to fetch registered users', error);
    return NextResponse.json(
